                    ...
                    <?php while ($f = mysqli_fetch_assoc($res)): ?>
                        <tr>
                            <td style="font-size: 0.8rem; opacity: 0.7;">
                                <?php echo date('d/m/Y', strtotime($f['fecha_reporte'])); ?></td>
                            <td style="font-weight: 600;">
                                <?php echo $f['usuario_nombre'] ? htmlspecialchars($f['usuario_nombre']) : 'Invitado'; ?>
                            </td>
                            <td style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                <?php echo htmlspecialchars($f['asunto']); ?></td>
                            <td>
                            ...
                            <td>
                                <div style="display: flex; gap: 10px; justify-content: center; align-items: center;">
                                    <a href="gestion.php?id=<?php echo $f['id_incidencia']; ?>" class="btn-admin"
                                        style="width: auto; margin-top: 0; padding: 5px 15px; font-size: 0.7rem;">Gestionar</a>
                                    <button onclick="confirmarEliminar(<?php echo $f['id_incidencia']; ?>)"
                                        class="btn-admin"
                                        style="width: auto; margin-top: 0; padding: 5px 10px; border-color: #ff4444; color: #ff4444;">🗑️</button>
                                </div>
                            </td>
                        </tr>
                    <?php endwhile; ?>
